...
function Header() {
   const routes = [
      ['Schedule',"/"], ['Events',"/events"], ['Competitions',"/competitions"], ['CA portal',"http://ca.elan.org.in"], ['Team',"/team"], ['Social Cause',"/socialcause"], ['Sponsors',"/sponsors"], ['Contact us',"/contact"]]
   return (
            ...
            <div className="hidden flex-row-reverse justify-between my-auto text-white list-none w-5/6 md:flex px-5">
               {
                  routes.reverse().map((route) => (
                     <Link to={route[1]} className='cursor-pointer font-heading font-bold text-xl'>{route[0].toUpperCase()}</Link>
                  ))
               }
            </div>
         </div>
